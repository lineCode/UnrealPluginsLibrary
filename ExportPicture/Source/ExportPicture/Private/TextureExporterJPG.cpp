...
UTextureExporterJPG::UTextureExporterJPG(const FObjectInitializer& OI)
	: Super(OI)
{
	SupportedClass = UTexture2D::StaticClass();
	PreferredFormatIndex = 0;
	FormatExtension.Add(TEXT("JPG"));
	FormatDescription.Add(TEXT("JPG"));
}
...
bool CanUncompressJPG(const FTextureSource& SourceData)
{
...
bool UTextureExporterJPG::SupportsObject(UObject* Object) const
{
	if (Super::SupportsObject(Object))
	{
		const UTexture2D* Texture = Cast<UTexture2D>(Object);
		return Texture && CanUncompressJPG(Texture->Source);
	}

	return false;
}

bool UTextureExporterJPG::ExportBinary(UObject* Object, const TCHAR* Type, FArchive& Ar, FFeedbackContext* Warn, int32 FileIndex /*= 0*/, uint32 PortFlags /*= 0*/)
{
	UTexture2D* Texture = CastChecked<UTexture2D>(Object);
	if (!CanUncompressJPG(Texture->Source))
	{
		return false;
	}
	...
	TSharedPtr<IImageWrapper> JPGImageWrapper = ImageWrapperModule.CreateImageWrapper(EImageFormat::JPEG);

	TArray64<uint8> RawData;
	Texture->Source.GetMipData(RawData, 0);

	if (JPGImageWrapper.IsValid() && JPGImageWrapper->SetRaw(RawData.GetData(), RawData.GetAllocatedSize(), Texture->Source.GetSizeX(), Texture->Source.GetSizeY(), ExportFormat, ExportBitDepth))
	{
		const TArray64<uint8>& PNGData = JPGImageWrapper->GetCompressed(100);
		if (PNGData.Num() != 0)
		{
